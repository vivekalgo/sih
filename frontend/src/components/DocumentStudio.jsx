  ...
  const purposeOptions = [
    {
      ...
      detail: 'Maximum privacy. Hides all Aadhaar, PAN, SSN, phone numbers, and financial details.'
    },
    {
      id: 'Bank KYC',
      label: 'Bank KYC',
    ...
    <div className="space-y-6 animate-fade-in max-w-6xl mx-auto">
      ...
      {currentStep === 1 && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="text-center max-w-2xl mx-auto space-y-2">
            <div className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full bg-cyan-50 dark:bg-cyan-950/60 border border-cyan-300 dark:border-cyan-500/30 text-cyan-700 dark:text-cyan-300 text-xs font-semibold">
              <Sparkles className="w-3.5 h-3.5" />
              <span>Step 1: Intelligent Policy Engine</span>
            </div>
            <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">
              Why are you sharing this document?
            </h2>
            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400">
              Select your use case. PrivacyGuard AI automatically configures what sensitive data is safe to keep versus what should be surgically masked.
            </p>
          </div>
            ...
            {purposeOptions.map((opt) => {
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-2xl border text-left cursor-pointer transition-all duration-200 relative flex flex-col justify-between group ${
                    isSelected
                      ? 'bg-gradient-to-br from-cyan-50 to-blue-50/80 dark:from-cyan-950/60 dark:to-slate-900 border-cyan-500 text-slate-900 dark:text-white shadow-lg shadow-cyan-500/10 ring-2 ring-cyan-500/40'
                      : 'bg-white dark:bg-slate-900/70 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900 shadow-sm'
                  }`}
                >
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      {isSelected ? (
                        <div className="flex items-center space-x-1 px-2 py-0.5 rounded-full bg-cyan-600 text-white text-[10px] font-bold">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] font-medium text-cyan-700 dark:text-cyan-300 mt-0.5">
                        {opt.desc}
                      </p>
                    </div>
          ...
          <div className="flex items-center justify-center pt-2">
            <button
              ...
              className="w-full sm:w-auto px-8 py-3 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold text-sm shadow-lg shadow-cyan-600/25 flex items-center justify-center space-x-2 transition-all hover:scale-[1.02]"
            >
      ...
      {currentStep === 2 && (
        <div className="space-y-6 animate-fade-in max-w-4xl mx-auto">
          ...
          {/* Step Header with Purpose Badge & Back Button */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm">
            <div className="flex items-center space-x-2.5">
              <button
                ...
                title="Go back to purpose selection"
              >
                <ArrowLeft className="w-4 h-4" />
              </button>
              <div>
                <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Step 2: Document Ingestion</span>
                <div className="text-xs font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                  Sharing Purpose: <strong className="text-cyan-600 dark:text-cyan-400 px-2 py-0.5 rounded bg-cyan-50 dark:bg-cyan-950/70 border border-cyan-300 dark:border-cyan-700/50">{purpose || 'General Sharing'}</strong>
                </div>
              </div>
            </div>

            <div className="flex items-center space-x-2">
              <span className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-1 bg-emerald-50 dark:bg-emerald-950/60 px-2.5 py-1 rounded-full border border-emerald-500/30">
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-3xl p-8 sm:p-12 text-center cursor-pointer transition-all duration-300 relative group overflow-hidden ${
                isDragOver
                  ...
                  : 'border-slate-300 dark:border-slate-700/80 bg-white/70 dark:bg-slate-900/40 hover:border-cyan-500/70 hover:bg-cyan-50/20 dark:hover:bg-slate-900/70'
              }`}
            >
              <input
                type="file"
              ...
              <div className="space-y-4 relative z-10 max-w-lg mx-auto">
                <div className="w-16 h-16 rounded-2xl bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center mx-auto text-cyan-600 dark:text-cyan-400 group-hover:scale-110 transition-transform shadow-md shadow-cyan-500/10">
                  {isProcessing ? (
                ...
                <div className="space-y-1.5">
                  <h3 className="text-base sm:text-lg font-extrabold text-slate-900 dark:text-white">
                    {isProcessing ? 'Processing & Scanning Document...' : 'Drop your document here, or click to browse'}
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Supports <strong>PDF</strong>, <strong>PNG</strong>, <strong>JPG/JPEG</strong>, and <strong>TXT</strong> files
                  </p>
                </div>

                {/* Animated Progress Bar during Scan */}
                {isProcessing && (
                  <div className="space-y-2 pt-2 animate-fade-in max-w-md mx-auto">
                    <div className="flex items-center justify-between text-xs font-semibold text-cyan-600 dark:text-cyan-400">
                      <span>Analyzing PII & Privacy Rules...</span>
                ...
                {/* Format Badges & Sample Button */}
                {!isProcessing && (
                  <div className="flex flex-wrap items-center justify-center gap-2 pt-3">
                    <span className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-[11px] font-mono text-slate-600 dark:text-slate-400 font-semibold border border-slate-200 dark:border-slate-700">
                      📄 Multi-page PDF
                    </span>
                    <span className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-[11px] font-mono text-slate-600 dark:text-slate-400 font-semibold border border-slate-200 dark:border-slate-700">
                      🖼️ ID Card Images
          ...
          {/* Tab Content B: Direct Text Paste */}
          {activeUploadTab === 'paste' && (
            <form onSubmit={handlePastedSubmit} className="space-y-4">
              <div className="glass-card rounded-3xl p-5 border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm bg-white dark:bg-slate-900/60">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                    <FileCode className="w-4 h-4 text-cyan-600 dark:text-cyan-400" />
                    Paste Raw Text / Customer PII / Medical Notes
                  </span>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-2xl p-4 text-xs font-mono text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500"
                />
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <button
                    ...
                    className="px-5 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold shadow-md flex items-center space-x-1.5 disabled:opacity-50 transition-all"
                  >
                    {isProcessing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
                    <span>Scan & Redact Text</span>
                  </button>
                </div>
              </div>
            </form>
          ...
          <div className="flex items-center justify-center p-3 rounded-2xl bg-slate-100/70 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400 gap-2">
            <span>Want to test quickly without uploading a personal file?</span>
            <button
              type="button"
              onClick={handleLoadSample}
              className="text-cyan-600 dark:text-cyan-400 font-bold hover:underline"
            >
              Click here to test with Sample ID Proof
            </button>
          </div>
      ...
      {currentStep === 3 && uploadResult && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm">
            <div className="flex items-center space-x-3">
              <button
                ...
                onClick={() => setCurrentStep(2)}
                className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors"
                title="Go back to document upload"
              >
                <ArrowLeft className="w-4 h-4" />
              </button>
              <div>
                <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Step 3: Redaction Strategy & Rules</span>
                <div className="text-xs font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                  ...
                  <span className="text-cyan-600 dark:text-cyan-400">{purpose || 'General Sharing'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white text-xs font-bold shadow-md shadow-cyan-600/20 flex items-center space-x-1.5 transition-all"
              >
                {isRedacting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Eye className="w-3.5 h-3.5" />}
                <span>Generate Clean Document (Step 4)</span>
                <ArrowRight className="w-3.5 h-3.5" />
              </button>
            </div>
          </div>

          {/* 1. MASKING STRATEGY SELECTOR (Descriptive Tag, Blackout Box, Scrambled Code, Synthetic Data) */}
          <div className="glass-card rounded-2xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm bg-gradient-to-r from-cyan-950/20 via-slate-900/40 to-slate-900/20 space-y-3">
              ...
              {maskingStrategies.map((strat) => {
                ...
                return (
                  <button
                    ...
                    className={`text-left p-3.5 rounded-2xl border text-xs transition-all relative flex flex-col justify-between ${
                      isSelected
                        ? 'border-cyan-500 bg-cyan-50 dark:bg-cyan-950/70 text-cyan-950 dark:text-cyan-100 shadow-md ring-2 ring-cyan-500/30'
                  ...
                  >
                    <div>
                      ...
                      <div className="text-[11px] font-mono text-cyan-700 dark:text-cyan-300 font-semibold bg-white/70 dark:bg-black/40 px-2 py-1 rounded-md border border-cyan-200 dark:border-cyan-800/60 truncate">
                        {strat.desc}
                      </div>
                    </div>
          ...
          {/* 2. SMART PII DETECTION & KEEP/HIDE CONTROLS */}
          <SmartRedactionPanel
          ...
          {/* 3. CUSTOM KEYWORDS & SECRET WORDS ACCORDION */}
          <div className="glass-card rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
            ...
            >
              <span className="flex items-center gap-2">
                ...
                <span>Add Custom Words, Project Codes or Secret Phrases to Hide</span>
              </span>
              <span className="text-[11px] text-cyan-600 dark:text-cyan-400 font-normal">
            ...
            {showAdvanced && (
              <div className="p-4 space-y-4 bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 text-xs">
                <form onSubmit={handleAddKeyword} className="flex items-center space-x-2">
                  <input
                    type="text"
                    placeholder="Type custom secret word (e.g. Project Apollo, Internal Salary, Password)..."
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl text-xs font-semibold flex items-center space-x-1 transition-all disabled:opacity-40"
                  >
          ...
          {/* Action Bar for Step 3 */}
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
            <button
              type="button"
              onClick={() => setCurrentStep(2)}
              className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold flex items-center justify-center space-x-1.5 transition-colors"
            >
              <ArrowLeft className="w-4 h-4" />
      ...
      {currentStep === 4 && uploadResult && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-xs text-slate-700 dark:text-slate-300 shadow-sm">
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold transition-colors flex items-center space-x-1"
              >
                <SlidersHorizontal className="w-3.5 h-3.5" />
                <span>Tweak Redaction Options</span>
              </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7 space-y-4">
              <div className="glass-card rounded-2xl p-4 border border-slate-200 dark:border-slate-800 space-y-3 shadow-lg bg-white dark:bg-slate-900/80">
                
                {/* Header with Page Jump & Zoom Controls */}
                <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-600 dark:text-slate-400 font-medium pb-2 border-b border-slate-200 dark:border-slate-800/80">
                  <span className="flex items-center gap-1.5 font-bold text-slate-800 dark:text-slate-200">
                    <ShieldCheck className="w-4 h-4 text-emerald-500" />
                    Clean Sanitized Preview ({totalPages} {totalPages > 1 ? 'Pages' : 'Page'})
                  </span>
                ...
                {previewPages.length > 0 ? (
                  <div
                    ...
                    className="rounded-xl overflow-y-auto max-h-[580px] p-3 space-y-4 bg-slate-100/70 dark:bg-[#07090f] border border-slate-200 dark:border-slate-800 shadow-inner"
                  >
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl font-mono text-xs text-slate-800 dark:text-slate-200 whitespace-pre-wrap max-h-[500px] overflow-y-auto">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
                  ...
                  <button
                    ...
                    disabled={isRedacting}
                    className="w-full sm:w-auto px-6 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-xs shadow-lg shadow-emerald-600/20 flex items-center justify-center space-x-2 transition-all hover:scale-[1.02]"
                  >
            ...
            <div className="lg:col-span-5 space-y-4">
              ...
              {/* Sub-tab 1: Security Badge & Zero-Leak Hash */}
              {outputTab === 'preview' && (
                <div className="space-y-4">
                  ...
                  {/* Summary of masked items */}
                  <div className="glass-card rounded-2xl p-4 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 space-y-2 text-xs">
